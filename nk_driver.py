...
def parse_args():
    ...
    parser.add_argument(
        '--resume', help="resume Newton's method from solver's saved state",
        action='store_true', default=False)
    parser.add_argument(
        '--rewind', help="rewind last step to recover from error", action='store_true',
        default=False)
...
def main(args):
    ...
    logging.basicConfig(
        filename=solverinfo['logging_fname'], filemode='a' if args.resume else 'w',
        format=logging_format, level=solverinfo['logging_level'])
